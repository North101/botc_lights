...
def advertising_payload(limited_disc=False, br_edr=False, name=None, services=None, appearance=0):
    ...
    if name:
        payload += _pack(_ADV_TYPE_NAME, name)

    if services:
        for uuid in services:
            b = bytes(uuid)
            if len(b) == 2:
                payload += _pack(_ADV_TYPE_UUID16_COMPLETE, b)
            elif len(b) == 4:
                payload += _pack(_ADV_TYPE_UUID32_COMPLETE, b)
            elif len(b) == 16:
                payload += _pack(_ADV_TYPE_UUID128_COMPLETE, b)
